...
/**
 * REST controller for managing [org.jakoboesterling.test.domain.Region].
 */
@RestController
@RequestMapping("/api")
class RegionResource(
     ...
     */
    @PostMapping("/regions")
    fun createRegion(@RequestBody region: Region): ResponseEntity<Region> {
        ...
        val result = regionService.save(region)
     ...
     */
    @PutMapping("/regions")
    fun updateRegion(@RequestBody region: Region): ResponseEntity<Region> {
    ...
    fun getRegion(@PathVariable id: Long): ResponseEntity<Region> {
        ...
        val region = regionService.findOne(id)
